...
void draw ( ) {  
  ...
  textSize(value);
  text("WHOA", 20, value);
  fill(255, 0, 0); //back to red
  textSize(32); // back to normal size
} 

// if input value is 1 (from ESP32, indicating a button has been pressed), change the background
void drawBackground() {
   if( switchValue == 1 )
    background(204, 246, 252); //light blue
  else
    background(251, 212, 255); //light pink
}

//-- animate ball left to right, use potValue to change speed
void drawBall() {
    if( switchValue == 1 )
    text("goodbye world", yBallPos, xBallPos); //text
    
   else
    fill(255,0,0); // red
    textSize(32); //size
    text("hello world", xBallPos, yBallPos); //text
...
void drawSmileFrown() {
    if( switchValue == 0 ){
    strokeWeight(2);
    fill(255, 249, 201); //yellow
    ellipse(width/2, height/2, 200, 250); //head
    fill(255); //white
    ellipse(350, height/2, 50, 50); //left eye
    ellipse(450, height/2, 50, 50); //right eye
    fill(value); //black
    ellipse(350, height/2, 20, 20); //left pupil
    ellipse(450, height/2, 20, 20); //right pupil
    
    line(375, 375, 425, 375); //mouth
}
    else{
    strokeWeight(2);
    fill(255, 249, 201); //yellow
    ...
    line(375, 375, 425, 375); //mouth
    line(330, 250, 375, 275); //left eyebrow
    line(425, 275, 475, 250); //right eyebrow
}
}
